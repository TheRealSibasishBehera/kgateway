...
// testingSuite is a suite of tests for ExtProc functionality
...
func NewTestingSuite(ctx context.Context, testInst *e2e.TestInstallation) suite.TestingSuite {
	...
	testCases := map[string]*base.TestCase{
		...
		"TestExtProcWithFailOpen": {
			Manifests: []string{
				gatewayTargetRefFailOpenManifest,
			},
		},
	}
...
// TestExtProcWithFailOpen tests ExtProc with failOpen enabled
func (s *testingSuite) TestExtProcWithFailOpen() {
	s.TestInstallation.Assertions.EventuallyGatewayCondition(
		s.Ctx,
		gateway.Name,
		gateway.Namespace,
	...
	testCases := []struct {
		...
		{
			name: "request succeeds with failOpen enabled",
			opts: []curl.Option{
				curl.WithHost(kubeutils.ServiceFQDN(gatewayService)),
				curl.VerboseOutput(),
				curl.WithHostHeader("failopen.example.com"),
				curl.WithPath("/"),
				curl.WithPort(8080),
				curl.WithHeader("instructions", getInstructionsJson(instructions{
					AddHeaders: map[string]string{"extproctest": "true"},
