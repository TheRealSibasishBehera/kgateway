...
echo "=== ExtAuth Test with Comprehensive Logging ==="
echo ""
...
LOG_DIR="$DEBUG_DIR/logs/extauth-${TIMESTAMP}"
mkdir -p "$LOG_DIR"
...
echo "Starting ExtAuth test from: $REPO_ROOT"
echo ""

(go test -tags e2e -v ./test/e2e/tests -run TestAgentgatewayIntegration/Extauth -timeout 30m 2>&1 | tee "$LOG_DIR/test-output.log") &
TEST_PID=$!
...
collect_logs "default" "gateway.networking.k8s.io/gateway-name=super-gateway" "gateway" 60 &
collect_logs "default" "app=ext-authz" "extauth" 60 &
collect_logs "default" "app=simple-svc" "backend" 60 &
...
cleanup() {
    ...
    echo "  - ExtAuth:    cat $LOG_DIR/extauth.log"
    echo "  - Backend:    cat $LOG_DIR/backend.log"
    echo ""
    echo "Quick analysis:"
    echo "  grep -i 'agentgatewaypolicy\\|extauth\\|ext_auth' $LOG_DIR/kgateway-controller.log"
    echo ""
}
